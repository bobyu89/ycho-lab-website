...
2. **檔名就是照片說明**，建議格式：`2026-06-15 EAFONS 研討會合影.jpg`
   - 檔名含「口試」「研討會／會議／論壇」「報告／發表／演講」等關鍵字時，活動花絮頁會自動分類，訪客可用標籤篩選；沒有關鍵字的歸入「其他」
...
- 幫論文加徽章（排名／作者角色／計畫編號）、幫計畫加中文描述與英文名稱、
...
| 想改什麼 | 改哪裡 |
|---|---|
| 首頁統計數字（成員數等） | `js/data.js` 的 `SITE.stats`（論文數/計畫數已自動計算） |
| 實驗室名稱／信箱／頁尾聯絡資訊 | `js/data.js` 的 `SITE` |
| 導覽列項目 | `js/components.js` 的 `NAV_ITEMS` |
| 論文徽章／非國科會計畫 | `js/enrich.js`（論文與計畫清單本身自動同步，見第五節） |
| 成員 | `members.html` |
| 老師簡介 | `pi.html` |
